...
{ 
   ...
   /** If Device is movable device Latitude/Longitude will update into locationbydevice columnfamily**/
    // System.out.println(rs1.getMetaData().getColumnCount());
	 if(resultset.getMetaData().getColumnCount()>6)
	 {
		 ...
		 //list of values in jstream
		 List valuelist = Arrays.asList(value);
		 //list of keys in jstream
		 ...
		 cql="insert into locationbydevice(key,latitude,longitude,status)values('"+key+"',"+latvalue+","+lonvalue+",'ON')";
		 st.executeUpdate(cql);
		 
	 }
	 System.out.println("CFNAME:"+cfname+"\n");
	/** insert Event Data into Standard column family***/
	 	cql="insert into e"+cfname+"(key1,ReceivedTime,year,month,streamdate,"+c+")values('"+Receivedtime+"','"+Receivedtime+"','"+year+"','"+month+"','"+streamdate+"',"+v+")";
	    ...
	    cql="insert into e"+cfname+"(key, Receivedtime)values('"+key+"','"+Receivedtime+"')";
		st.executeUpdate(cql);
		System.out.println("Inserted data into "+cfname +"  Columfamily");
	}	
   ...
   //Date date = new Date();
    ...
    /** insert Event Data into Standard column family***/
	 	cql="insert into "+cfname+"(key1,ReceivedTime,year,month,streamdate,"+c+")values('"+Receivedtime+"','"+Receivedtime+"','"+year+"','"+month+"','"+streamdate+"',"+v+")";
	    st.executeUpdate(cql);
	    System.out.println("Inserted data into "+cfname +"  Columfamily");
